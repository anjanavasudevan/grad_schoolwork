...
\end{align}
\subsection{Option 1:}
Let $\vec{A} = \myvec{x_1 & y_1 & z_1}^T$ and $\vec{B} = \myvec{x_1 & y_1 & z_2}^T \in \vec{S}$, and $k_1$ and $k_2$ be some scalars. As per definition:
\begin{align}
...
\subsection{Option 3:}
Option 3 is not a valid subspace of $\R^3$ as it can be shown that for some scalars $k_1$ and $k_2$, $\vec{A}$ and $\vec{B} \in \vec{S}$ in the option:
\begin{multline}
    \implies \myvec{1 & 1 & 0}\lcbrak{k_1\vec{A}} + \rcbrak{k_2\vec{B}} = \\
...
\begin{align}
  \myvec{1 & -1 & 0}\vec{A} = \myvec{1 & -1 & 0}\vec{B} = 1
\end{align}
Therefore, Options 1 and 2 are valid subspaces of the vector space $\R^3$
\end{document}
