...
\maketitle
\begin{abstract}
This document aims to plot equation of tangent to a circle givenits parallel line using matrices
\end{abstract}
...
%
\section{Question}
Find equation of the tangent to the circle
\begin{align}
  x^2 + y^2 = 4 \label{eq:1}
\end{align}
...
\end{align}
\section{Answer}
The equations for the circle and line in \eqref{eq:1} and \eqref{eq:2} can be rewritten in vector form as:
\begin{align}
  \norm{\vec{x}}^2 = 4 \\
  \myvec{1 & 2}\vec{x} = 6 \label{eq:3}
\end{align}
The center of the circle happens to be $(0, 0)$ \\
The equation of a line is of the form:
\begin{align}
  \vec{n}^T\vec{x} = c \label{eq:4}
\end{align}
Where n is the normal to the line. \\
Comparing \eqref{eq:4} to \eqref{eq:3},
\begin{align}
  \vec{n} = \myvec{1 \\ 2}
\end{align}
Since the tangent is parallel to the line in \eqref{eq:3}, it will also have the same normal.

The point of contact for a conic is given by:
\begin{align}
  \vec{v} = \vec{V}^{-1}(\kappa\vec{n}-\vec{u}) \label{eq:5}
\end{align}
where,
\begin{align}
  \kappa = \pm \sqrt[]{\frac{\vec{u}^T\vec{V^{-1}u}-f}{\vec{n}^T\vec{V}^{-1}\vec{n}}} \label{eq:6}
\end{align}
For a circle,
\begin{align}
  \vec{V} = \vec{I}
...
\begin{align}
  ...
  \vec{IX} = \vec{X}
\end{align}
Therefore \eqref{eq:5} and \eqref{eq:6} simplify to:
\begin{align}
  \kappa = \pm \sqrt[]{\frac{\vec{u^Tu}-f}{\vec{n^T}\vec{n}}} \\
...
\end{align}
Since there are two points of contact, there are two tangents parallel to \eqref{eq:3} that have the same normal vector.
\begin{align}
  \implies \vec{n^Tq_1} = c_1 \\
  \vec{n^Tq_2} = c_2
\end{align}
Substituting the values, we get:
\begin{align}
